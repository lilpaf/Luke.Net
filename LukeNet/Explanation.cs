...
namespace Lucene.Net.LukeNet
{
	...
	public class ExplanationDialog : System.Windows.Forms.Form
	{
        ...
        {
            string value = e.Value.ToString("G4"); // Get the value directly from the property
            string description = e.Description; // Get the description directly from the property

            TreeNode node = new TreeNode(value + "  " + description);
            ...
            {
                foreach (Explanation kid in kids)
                {
                    AddNode(node, kid);
                }
            }
